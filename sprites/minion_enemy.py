...
class MinionEnemy(BaseEnemy):
    def __init__(self, enemy_type, sprites, player_center, center, x_velocity, y_velocity, number_of_images, scaled_width, scaled_height):
        super(MinionEnemy, self).__init__(enemy_type, sprites, player_center, center, x_velocity, y_velocity, number_of_images,
    ...
    def enemy_controller_join(self, dt):
        ...
        if self.join_count > 3:
            ...
            self.rect.clamp_ip(pg.Rect((self.rect.left, self.y_start), (self.rect.width, self.rect.height)))
    ...
    def enemy_controller_dive(self, dt):
        if self.rect.bottom > (constants.SCREEN_HEIGHT - self.rect.height):
            ...
            self.attacking = True
            if self.enemy_type == EnemyType.MINION_2:
                dx = self.player_center[0] - self.rect.centerx
                dy = self.player_center[1] - self.rect.centery
                number_of_steps = self.weird_division(dy, self.y_velocity)
